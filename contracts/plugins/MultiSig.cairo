...
const SIG_LENGTH = 3 # sig length. here sig should be sig_r, sig_s and public key
####################
# EVENTS
####################
...
func validate{
    ...
    if call_array_len == 1:
        if call_array[0].to == tx_info.account_contract_address:
            ...
            clear_confirmations(tx_info.signature_len, tx_info.signature)
            return()
        end
        else:
...
func is_valid_signature{
    ...
    if is_signer_sig_valid == TRUE:
        return is_valid_signature(hash, sig_len - SIG_LENGTH, sig + SIG_LENGTH)
    end
    return (FALSE)
...
func validate_signer_signature{
    ...
    let (is_caller_owner) = is_owner(address=signatures[2])
    let (has_signer_already_signed) = has_signed(address=signatures[2])
    ...
    with_attr error_message("signer signature invalid"):
        ...
        verify_ecdsa_signature(
            message=message,
            public_key=signatures[2],
            signature_r=signatures[0],
            signature_s=signatures[1])
    end
    _has_signed.write(address=signatures[2], value=TRUE)
...
func clear_confirmations{
    ...
    _has_signed.write(address=sig[2], value=FALSE)
    clear_confirmations(sig_len -3, sig + 3)
    return ()
end
